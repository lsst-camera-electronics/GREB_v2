...
entity GREB_v2_cmd_interpeter is
  ...
  port (
    ...
    image_pattern_read : in    std_logic_vector(1 downto 0);  -- this register gives the state of image patter gen. 1 is ON
    ...
    seq_override_wr    : out   std_logic_vector(1 downto 0);
    seq_override_rd    : in    Slv32Array(1 downto 0);
...
end entity GREB_v2_cmd_interpeter;
...
architecture Behavioral of GREB_v2_cmd_interpeter is
  ...
  -- Sequencer Defaults
  signal next_seq_override_wr : std_logic_vector(1 downto 0);
...
begin
  ...
  process (pres_state, regReq, regOP, RegAddr, regdatawr_masked, regwren, switch_addr, time_base_actual_value,
    ...
    -- Bacbias switch
           ...
           mgt_avcc_ok, image_size, image_pattern_read, ccd_sel_read, statusreg, ccd_oe_read,
           sync_cmd_delay_read,
           interrupt_mask_read,
           seq_time_mem_readbk, seq_out_mem_readbk, seq_prog_mem_readbk, seq_ind_func_mem_rdbk,
           seq_ind_rep_mem_rdbk, seq_ind_sub_add_mem_rdbk, seq_ind_sub_rep_mem_rdbk, seq_op_code_error,
           seq_op_code_error_add, seq_override_rd, enable_conv_shift_in, start_add_prog_mem_rbk,
  ...
  begin
    ...
    case pres_state is
        ...
        if (regReq = '0') then
          ...
          if (regOp = '0') then
            ...
            -- function time write
              ...
              if sequencer_index(regAddr(13 downto 12)) < cfg.numSequencers then
      ...
      when read_image_pattern_mode_state =>
        ...
        next_state     <= wait_end_cmd;
        next_regAck    <= '1';
        next_regDataRd <= x"0000000" & "00" & image_pattern_read;
